...
1. **Identity**: every principal (human or agent) holds an ed25519 keypair; enrollment is dual-control (the founding ceremony) or owner-attested (agents); the ceremony IS the founders' mutual introduction. Day zero is one command: `ostrom start` builds a local org (hub, key, ceremony, TUI); `ostrom invite` prints a join blob and `ostrom join <blob>` lands the member on the roster with a real enrollment (sponsor-signed, level-capped), not a guest shim.
...
4. **The pool**: tasks are offered (open or assigned, optionally declaring a skill); the workers' gates declare presence; on dispatch the gate claims under the agent's grants and launches the sandboxed session; the agent works (broker, staged harness, skills, memory) and can ask its owner questions with a human-scale answer window; the owner reviews and merges, and a signed receipt mints. Rejection returns work to the worker (revision 2+). Humans can settle a task end to end without an agent when they want to.
5. **Consent and autonomy**: suggestions pass a consent gate; policy is pure, deny-wins; trust is tiered and expiring; circuit breakers and a kill switch are the safety net (fired and verified live). The cockpit answers one question, "what needs me?", on a single inbox surface: contact requests, consent proposals, tasks, and agent asks, one confirm law for all of them.
...
**Running it**: a local org is `ostrom start`; going real is `ostrom start --deploy --host <fqdn>` (a complete compose bundle with generated secrets, host TLS, a hardened systemd unit, and a promotion runbook); unattended survival is `--service`. Hub state is integrity-anchored: every sweep signs a checkpoint over the audit tail, mailbox cursors, and task board, and clients detect any rewind. The words are a law too: [the vocabulary](https://github.com/ostrom-project/ostrom/blob/main/docs/VOCABULARY.md) keeps every user-facing string in plain English, enforced by CI.

## Status

**The pool runtime is live and journey-proven**: the full pooled work loop (offer, presence, dispatch, sandboxed activation, work, review, receipt, rework), the day-zero journey, failure honesty, the safety net fired deliberately, memory closure (prior work informing new work), reboot survival, and skills-in-pool. All proven live on a two-machine pilot with real LLM agents in real sandboxes. **v2 federation is complete**: the member bridge (bilateral peering, cross-hub addressing, signed delivery verdicts, the contact gate), the directory (the OrgCard as a CAS record + the bounded verified listing), the charter v2 policy payload, and the org narrative with the two-consent publication law. The journey traces are public in the [main repository](https://github.com/ostrom-project/ostrom/tree/main/docs/pilot).

**The usability program is shipped** (2026-09-19 to 2026-09-21): one-command orgs and enrollment-backed joins, the TUI feel pass (connectivity verdicts, loud failures, progressive disclosure, the Enter law), the unified inbox, the id and identity laws, the one-command deploy path with integrity anchoring, and the plain-English vocabulary law; 1870 tests, CI green with scripted end-to-end acceptance runs on every push.
